...
async function upload() {
  try {
    ...
    console.log("Transforming and uploading tasks...");
    ...
    for (const week of data.weeks) {
      for (const day of week.days) {
        ...
        for (const taskItem of day.tasks) {
          const focusbookTask = {
            id: uuidv4(),
            text: `[${week.title} - ${day.title}] ${taskItem.text}`,
            done: false,
            area: "career",
            priority: "medium",
            date: taskDate,
    ...
    console.log(`Upload complete! Inserted ${totalAdded} tasks into your FocusBook app.`);
    process.exit(0);
  } catch (error) {
